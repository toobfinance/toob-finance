...
export abstract class AlgebraBaseProvider extends LiquidityProvider {
  ...
  async fetchPoolsForToken(
    ...
    let multicall3 = this.client.chain?.contracts?.multicall3
      ?.address as Address;
    if (multicall3 === undefined && this.client.chain?.id == 1996) {
      multicall3 = "0xB32d192e07727A0F6f48e721791aa4e02cd292e1" as Address;
    }
    ...
    staticPools.forEach((pool, i) => {
      ...
      const price = (globalState[i].result as any)?.[0];
      const tick = (globalState[i].result as any)?.[1];
