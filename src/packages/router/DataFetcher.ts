...
export class DataFetcher {
  ...
  startDataFetching(
    ...
    if (this._providerIsIncluded(LiquidityProviders.SushiSwapV2, providers)) {
      try {
        const provider = new SushiSwapV2Provider(this.chainId, this.web3Client2)
    ...
    if (this._providerIsIncluded(LiquidityProviders.SushiSwapV3, providers)) {
      try {
        const provider = new SushiSwapV3Provider(this.chainId, this.web3Client3)
        this.providers.push(provider)
      } catch (e: unknown) {
        console.warn(e)
      }
    ...
    if (this._providerIsIncluded(LiquidityProviders.UniSwapV3, providers)) {
      try {
        const provider = new UniswapV3Provider(this.chainId, this.web3Client2)
        this.providers.push(provider)
      } catch (e: unknown) {
        console.warn(e)
      }
    ...
    if (this._providerIsIncluded(LiquidityProviders.TraderJoe, providers)) {
      try {
        const provider = new TraderJoeProvider(this.chainId, this.web3Client2)
        this.providers.push(provider)
      } catch (e: unknown) {
        console.warn(e)
      }
